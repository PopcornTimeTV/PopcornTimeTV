...
extension Bundle {
    ...
    var localizedVersion: String {
        ...
        let version = [bundle.object(forInfoDictionaryKey: "CFBundleShortVersionString"), bundle.object(forInfoDictionaryKey: "CFBundleVersion")].compactMap({$0 as? String}).joined(separator: ".").components(separatedBy: ".")
        return version.compactMap {
            if let value = Int($0) {
