    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        if tableView == firstTableView && subtitles.isEmpty {
            ...
            if subtitles.count > 0 {
                let englishLang = Locale.current.localizedString(forLanguageCode: "en")!.localizedCapitalized
                subtitlesInView = [currentSubtitle ?? subtitles[englishLang]?.first ?? subtitles[subtitles.keys.first!]!.first!,Subtitle(name: "", language: "Select Other".localized, link: "", ISO639: "", rating: 0.0)]
            }
            
            //insert predetermined subtitle or english or first available whichever exists
