...
class SubtitlesViewController: OptionsStackViewController,SubtitlesViewControllerDelegate, UITableViewDataSource {
    ...
    func numberOfSections(in tableView: UITableView) -> Int {
        ...
        if tableView == firstTableView && subtitles.isEmpty {
            ...
            subtitlesInView = [
                currentSubtitle
                    ?? subtitles[Locale.current.localizedString(forLanguageCode: "en")!.localizedCapitalized]?.first
                    ?? subtitles[subtitles.keys.first!]!.first!
                , Subtitle(name: "", language: "Select Other".localized, link: "", ISO639: "", rating: 0.0)
