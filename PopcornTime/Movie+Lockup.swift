...
extension Movie {
    ...
    var lockUp: String {
        var string = "<lockup actionID=\"showMovie»\(title.cleaned)»\(id)\" playActionID=\"playMedia»\(Mapper<Torrent>().toJSONString(torrents)?.cleaned ?? "")»\(Mapper<Movie>().toJSONString(self)?.cleaned ?? "")\">"
